...
def setup_model_and_optimizer(
    ...
    *,
    ...
    if getattr(args, 'gtp_runtime_profile', False):
        if not is_gtp_remat_active(args):
            raise ValueError('--gtp-runtime-profile requires GTP weight rematerialization')
        if getattr(args, 'cuda_graph_impl', 'none') != 'none':
            raise ValueError('--gtp-runtime-profile currently supports eager execution only')
        ...
        configure_gtp_runtime_profiler(
            GTPRuntimeProfileConfig(
                warmup_iters=args.gtp_runtime_profile_warmup_iters,
                profile_iters=args.gtp_runtime_profile_iters,
                log_dir=Path(args.gtp_runtime_profile_log_dir),
            ),
            model=model,
        )
