...
class _ReduceScatterWithFP32AccumulationWorkHandle:
    ...
    def wait(self):
        ...
        a2a_output = self.all_to_all_output_tensor.view((self.world_size, -1))
        if self.fp32_accumulation_output_tensor is None:
            output_tensor_in_fp32 = torch.sum(a2a_output, dim=0, dtype=torch.float32)
        else:
            output_tensor_in_fp32 = self.fp32_accumulation_output_tensor
            torch.sum(a2a_output, dim=0, dtype=torch.float32, out=output_tensor_in_fp32.view(-1))
...
def reduce_scatter_with_fp32_accumulation(
    ...
    if fp32_accumulation_output_tensor is not None:
        assert fp32_accumulation_output_tensor.shape == output_tensor.shape, (
            f"fp32_accumulation_output_tensor shape "
            f"{tuple(fp32_accumulation_output_tensor.shape)} does not match output_tensor shape "
